...
const WorkerHome = () => {
  ...
  const [data,setData]=useState([])
    useEffect(()=>{
        axiosinstance(`/submission?email=${user?.email}`)
        .then(res=>setTsubmission(res.data.length))
        .catch(err=>console.log(err))
         axiosinstance(`/submission?workermail=${user?.email}&status=pending`)
        .then(res=>setPsubmission(res.data.length))
        .catch(err=>console.log(err))
         axiosinstance(`/total-payed?email=${user?.email}`)
        .then(res=>setTpayment(res.data.totalPayed))
        .catch(err=>console.log(err))
        axiosinstance(`/submission?workermail=${user?.email}&status=approve`)
